...
foreach ($themes as $theme_key => $theme_data) {
    $match = true;
    foreach ($theme_data as $section => $values) {
        if (!is_array($values)) continue;
        foreach ($values as $key => $value) {
            if (!isset($parsed_ini[$section][$key]) || $parsed_ini[$section][$key] != $value) {
                $match = false;
                break 2;
            }
        }
...
<html lang="en">
    <head>
        ...
        <script type="text/javascript">
            ...
            function uploadCssBackup() { document.getElementById('cssUpload').submit(); }
                ...
                foreach($parsed_ini as $section=>$values_in_section) {
                    ...
                    foreach($values_in_section as $key=>$value) {
                        ...
                        elseif (strpos($key, 'FontSize') !== false) {
                            ...
                            echo "<div class='desc'>Value (px.)</div>";
                        }
                        elseif (strpos($key, 'LastHeardRows') !== false) {
                            echo "<input type='text' name='$input_name' value='$value_display' style='text-align:center;' />";
                            echo "<div class='desc'>Value (numeric; 100 max.)</div>";
                        }
                        else {
            ...
            <div class="profile-card">
                ...
                <div class="profile-body">
                    <form action="" method="post" name="edit-css" style="display:flex; flex-direction:column; gap:15px; align-items:center;">
                        ...
                        <select id="themeSelector" name="theme_selection" class="profile-select">
                        ...
                        <div style="display:flex; gap:10px; width:100%;">
                            <button type="submit" name="apply_theme" value="1" class="profile-btn" style="flex:1;">Apply Selected Theme</button>
                        ...
                        <div>
                            <form method="post" action="">
                                ...
                                <div class="toggle-group">
                                    <input type="radio" name="CallProvider" value="OHR" id="rad-rid" <?php if (isset($_SESSION['WPSDdashConfig']['WPSD']['CallLookupProvider']) && $_SESSION['WPSDdashConfig']['WPSD']['CallLookupProvider'] == "OHR") echo 'checked'; ?>>
                                    <label for="rad-rid">Outlaw</label>
                ...
                <div class="profile-body">
                    <form id="cssUpload" action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div style="position:relative; margin-bottom:15px;">
                            <input id="fileid" name="cssFile" type="file" class="inputfile" onchange="uploadCssBackup()" accept=".zip" />
