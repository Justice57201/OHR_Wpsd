...
<body>
    <div class="container">
        <div class="header">
            ...
            <h1><img src="/images/ohr.png" alt="Logo" style="height:80px; vertical-align:middle; margin-right:20px;">
								Outlaw <?php echo __('Dashboard for') . " <code style='font-weight:550;'>" . $_SESSION['MYCALL'] . "</code>"; ?></h1>
            <div id="CheckMessage">
            ...
            <div class="navbar">
                ...
                if ($_SERVER["PHP_SELF"] !== "/admin/index.php") {
                    ...
                    if ($osVer >= 12) {
                        echo '<a class="menuwifi" href="/admin/wifi-manager.php" style="display:none;">WiFi</a>' . "\n";
                    }
                }
                ?>
            </div>
        ...
        if (file_exists('/etc/dstar-radio.mmdvmhost') && !$isNewZumInstall) {
            echo '<div class="nav">' . "\n";
                    ...
                    echo '<script>
                      async function fetchData(url, targetElement) {
                        try {
                          ...
                          // Log HTTP errors (502, 500, 404, etc.)
                          if (!response.ok) {
                              console.warn(`Server error (${response.status}) fetching ${url}. Suppressing output.`);
                              return;
                          }

                          const data = await response.text();

                          // Safety check: sometimes webservers return 200 OK even for error pages
                          if (data.includes("502 Bad Gateway")) {
                              return;
                          }

                          // Only update if content changed (prevents flashing)
                          if ($(targetElement).html() !== data) {
                              $(targetElement).html(data);
                          }
                        } catch (error) {
                          // Log Network errors (e.g., server totally unreachable)
                          console.debug(`Network error fetching ${url}:`, error);
                        }
                      }
	    ...
	    if ($_SERVER["PHP_SELF"] !== "/admin/index.php") {
	        $loaderHTML = '<div class="loading-data-panel"><div class="loading-spinner"></div><span class="loading-text">Loading Live Data...</span></div>';
            ...
            if ($isNewZumInstall) {
                ...
                echo "<p>Please select an option below to get started:</p>\n";
            } else {
